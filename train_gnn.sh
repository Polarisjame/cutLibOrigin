...
dataset_dir='/data/zhoulingfeng/data/save_subgraph' # nn.Dataset save
log_dir='./log/train_log_DeepCutDelay_agg_rand_cell2cell_fanout_v2.log'
...
model_ckpt='.'
# model_ckpt='/data/zhoulingfeng/checkpoints/DeepCutDelay_agg_rand_cell2cell_fanout_batch32_epoch4_step14263.pth'
...
if false; then
    ...
    >log/erroroutDeepCutDelay_agg_rand_cell2cell_fanout_v2.log 2>&1 &
fi
...
if true; then
    ...
    nohup python3 -u src/get_label_samp_full.py\
        --n_layer ${n_layer}\
        --batch_size ${batch_size}\
        --num_worker 15\
    >log/sample_data.log 2>&1 &
fi